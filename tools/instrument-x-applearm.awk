...
function resume_runtime() {
  ...
  print "resume_pending_restore() { [ -z \"$RESUME_PENDING_SNAPSHOT\" ] || { resume_restore \"$RESUME_PENDING_SNAPSHOT\"; RESUME_HAVE_BASE=1; RESUME_PENDING_SNAPSHOT=; }; }"
  print "resume_begin() { local id hash state snapshot; id=$1; hash=$2; state=\"$RESUME_DIR/state/$id\"; snapshot=\"$RESUME_DIR/snapshots/$id\"; mkdir -p \"$RESUME_DIR/state\" \"$RESUME_DIR/snapshots\"; if [ -f \"$state\" ] && [ \"$(cat \"$state\")\" = \"$BUILD_STACK_REV $id $hash\" ] && [ -d \"$snapshot\" ]; then RESUME_PENDING_SNAPSHOT=$id; echo \"*** RESUME: reusing $id\"; return 1; fi; [ ! -f \"$state\" ] || echo \"*** RESUME: invalidating $id (recipe or build flags changed)\"; resume_invalidate \"$id\"; if [ \"$RESUME_HAVE_BASE\" -eq 0 ]; then resume_pending_restore; [ \"$RESUME_HAVE_BASE\" -eq 1 ] || { rm -rf \"$PREFIX\" \"$BLDDEP\" \"$BUILDD\"; mkdir -p \"$PREFIX\" \"$BLDDEP\" \"$BUILDD\"; }; fi; rm -rf \"$BUILDD\"; mkdir -p \"$BUILDD\"; echo \"*** RESUME: building $id\"; return 0; }"
...
END { if (reset) { print "error: unsupported x-applearm.sh reset block" > "/dev/stderr"; exit 2 }; emit_step(); print "resume_pending_restore" }
